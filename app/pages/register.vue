...
async function submitForm() {
  ...
  try {
    const res = await auth.requestRegisterOtp(form)
    if (res.ok) {
      // Chỉ tự điền OTP khi fallback về email mô phỏng local.
...
<template>
  <AuthShell @autofill="otpCode = $event">
    <!-- Step 1: form -->
    <form v-if="step === 'form'" autocomplete="off" class="flex flex-col gap-3 py-1" @submit.prevent="submitForm">
      ...
      <div class="flex flex-col gap-1 bg-slate-800/20 p-2.5 rounded-2xl border border-slate-800/40">
        ...
        <div class="relative">
          ...
          <input v-model="form.name" type="text" name="register-display-name" autocomplete="off" placeholder="VD: Trần Văn A" class="w-full border-0 bg-transparent pl-7 pr-3 text-sm text-slate-100 placeholder-slate-600 focus:outline-none focus:ring-0">
        </div>
      </div>
      ...
      <div class="flex flex-col gap-1 bg-slate-800/20 p-2.5 rounded-2xl border border-slate-800/40">
        ...
        <div class="relative">
          ...
          <input v-model="form.username" type="text" name="register-username" autocomplete="off" placeholder="viet_lien_khong_dau" class="w-full border-0 bg-transparent pl-5 pr-3 text-sm text-slate-100 placeholder-slate-600 focus:outline-none focus:ring-0">
        </div>
      </div>
      ...
      <div class="flex flex-col gap-1 bg-slate-800/20 p-2.5 rounded-2xl border border-slate-800/40">
        ...
        <div class="relative">
          ...
          <input v-model="form.email" type="email" name="register-email" autocomplete="off" placeholder="username@gmail.com" class="w-full border-0 bg-transparent pl-7 pr-3 text-sm text-slate-100 placeholder-slate-600 focus:outline-none focus:ring-0">
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-2">
        <div class="flex flex-col gap-1 bg-slate-800/20 p-2.5 rounded-2xl border border-slate-800/40">
          ...
          <input v-model="form.password" type="password" name="register-password-new" autocomplete="new-password" placeholder="••••••" class="w-full border-0 bg-transparent py-0.5 text-sm text-slate-100 placeholder-slate-600 focus:outline-none focus:ring-0 font-mono">
        </div>
        <div class="flex flex-col gap-1 bg-slate-800/20 p-2.5 rounded-2xl border border-slate-800/40">
          <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Nhập lại</label>
          <input v-model="form.confirm" type="password" name="register-password-confirm-new" autocomplete="new-password" placeholder="••••••" class="w-full border-0 bg-transparent py-0.5 text-sm text-slate-100 placeholder-slate-600 focus:outline-none focus:ring-0 font-mono">
        </div>
      </div>
      ...
      <button type="submit" :disabled="isSubmitting" class="w-full flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-indigo-600 via-indigo-700 to-violet-600 py-3.5 px-4 font-bold text-white shadow-lg transition hover:opacity-95 active:scale-95 disabled:cursor-not-allowed disabled:opacity-60 mt-1">
        <span>{{ isSubmitting ? 'Đang gửi mã...' : 'Gửi mã kích hoạt bảo mật' }}</span>
    ...
    <form v-else class="flex flex-col gap-4" @submit.prevent="verify">
      ...
      <button type="submit" :disabled="isSubmitting" class="w-full flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-600 via-emerald-700 to-teal-600 py-3.5 px-4 font-bold text-white shadow-lg transition hover:opacity-95 active:scale-95 disabled:cursor-not-allowed disabled:opacity-60">
        <span>{{ isSubmitting ? 'Đang kích hoạt...' : 'Kích hoạt & Vào trang chính' }}</span>
